...
- (void)awakeFromNib {
	plistPath = [@"~/Library/Services/SilverService.service/Contents/Info.plist" stringByExpandingTildeInPath];
	[plistPath retain];
	...
	if (![fm fileExistsAtPath:plistPath]) {
		...
		if (!plist) {
			...
			services = [[NSMutableDictionary alloc] init];
			NSEnumerator *enumerator = [[plist objectForKey:@"NSServices"] objectEnumerator];
			id s;
			while (s = [enumerator nextObject]) {
				...
				[[services objectForKey:message] addObject: [[s objectForKey:@"NSUserData"] propertyList]];
			}
		}
	}
}
		...
		while (service = [serviceEnum nextObject]) {
			...
			[serviceSpec setObject:
				[NSDictionary dictionaryWithObject:
					[@"SilverService/" stringByAppendingString:[service objectForKey:@"name"]]
																		forKey:@"default"]
																		forKey:@"NSMenuItem"];
			if (![[service objectForKey:@"in"] isEqualToString:@"none"]) {
				[serviceSpec setObject:[NSArray arrayWithObject:@"NSStringPboardType"] forKey:@"NSSendTypes"];
			}
